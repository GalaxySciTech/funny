...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-4">{h.shopTitle}</h2>
          <p className="text-slate-400 text-lg">{h.shopDesc}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {h.coinPackages.map((pkg, i) => {
            const colors = ["from-slate-600 to-slate-700", "from-brand-600 to-brand-700", "from-gold-500 to-amber-600"];
            return (
              <div key={i} className={`relative card p-8 text-center ${pkg.popular ? "ring-2 ring-brand-500 scale-105" : ""}`}>
                ...
                <div className={`w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br ${colors[i]} flex items-center justify-center text-2xl shadow-lg`}>
                  🪙
                </div>
                <div className="text-4xl font-black text-white mb-1">{pkg.coins}</div>
                <div className="text-slate-400 mb-2">{h.coins}</div>
